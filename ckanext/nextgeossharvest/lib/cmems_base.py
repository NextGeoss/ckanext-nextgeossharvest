...
class CMEMSBase(HarvesterBase):
    ...
    def _get_metadata_create_objects(self, start_date, end_date, start_date_to_inc, harvest_job, current_guids, current_guids_in_harvest):
        # Get contents
        try:
            ...
            sst_thumbnail = "http://cmems.isac.cnr.it/thredds/wms/METOFFICE-GLO-SST-L4-NRT-OBS-SST-V2?request=GetMap&version=1.3.0&layers=analysed_sst&crs=CRS:84&bbox=-180,-90,180,90&WIDTH=800&HEIGHT=800&styles=boxfill/rainbow&format=image/png&time=" + str(start_date) + 'T12:00:00.000Z'
            sic_north_thumbnail = "http://thredds.met.no/thredds/wms/sea_ice/SIW-OSISAF-GLO-SIT_SIE_SIC-OBS/ice_conc_north_aggregated?request=GetMap&layers=ice_conc&version=1.3.0&crs=CRS:84&bbox=-180,0,180,90&WIDTH=800&HEIGHT=800&styles=boxfill/rainbow&format=image/png&time=" + str(start_date) + "T12:00:00.000Z"
            sic_south_thumbnail = "http://thredds.met.no/thredds/wms/sea_ice/SIW-OSISAF-GLO-SIT_SIE_SIC-OBS/ice_conc_south_aggregated?request=GetMap&layers=ice_conc&version=1.3.0&crs=CRS:84&bbox=-180,-90,180,0&WIDTH=800&HEIGHT=800&styles=boxfill/rainbow&format=image/png&time=" + str(start_date) + "T12:00:00.000Z"
            ...
            for i in range(10):
                ...
                ocean_forecast_thumbnail = "http://thredds.met.no/thredds/wms/topaz/dataset-topaz4-arc-1hr-myoceanv2-be?request=GetMap&version=1.3.0&layers=temperature&CRS=CRS:84&bbox=-180,0,180,90&WIDTH=800&HEIGHT=800&styles=boxfill/rainbow&format=image/png&time=" + str(start_date_inc)

                r_status_code = self._crawl_urls_simple(ocean_forecast_thumbnail, 'cmems')

                if r_status_code == 200:
                    metadata = {}
                    forecast = [str(start_date_inc), ocean_forecast_thumbnail]  
                    ...
                    metadata['Coordinates'] = [[-180,90],[180,90],[180,63],[-180,63],[-180,90]]
                    metadata['downloadLink'] = "ftp://mftp.cmems.met.no/Core/ARCTIC_ANALYSIS_FORECAST_PHYS_002_001_a/dataset-topaz4-arc-myoceanv2-be/" + fyear + fmonth + fday + "_dm-metno-MODEL-topaz4-ARC-b" + year + month + day + "-fv02.0.nc"
                    ...
                    if hash in current_guids:
                        ...
                        obj = HarvestObject(job=harvest_job, guid=hash, extras=[
                            ...
                            HOExtra(key='identifier', value=metadata['identifier']),
                            HOExtra(key='download_link', value=metadata['downloadLink']),
                            HOExtra(key='dataset_name', value=metadata['datasetname']),
                            #HOExtra(key='original_document', value=xml_string),
                            HOExtra(key='original_metadata', value=json.dumps(metadata)),
                            HOExtra(key='original_format', value='netCDF')
                        ])
                                
                        obj.save()
                        ids.append(obj.id)
            
            cmems_collections = [sst_thumbnail, sic_north_thumbnail, sic_south_thumbnail]
            ...
            for collection in cmems_collections:
                ...
                if r_status_code == 200:
                    ...
                    if collection == sst_thumbnail:
                        ...
                        metadata['downloadLinkEase'] = "ftp://mftp.cmems.met.no/Core/SEAICE_GLO_SEAICE_L4_NRT_OBSERVATIONS_011_001/METNO-GLO-SEAICE_CONC-NORTH-L4-NRT-OBS/" + year + "/" + month + "/" + "ice_conc_nh_ease-125_multi_" + year + month + day + "1200.nc"
                        ...
                        metadata['downloadLinkEase'] = "ftp://mftp.cmems.met.no/Core/SEAICE_GLO_SEAICE_L4_NRT_OBSERVATIONS_011_001/METNO-GLO-SEAICE_CONC-SOUTH-L4-NRT-OBS/" + year + "/" + month + "/" + "ice_conc_sh_ease-125_multi_" + year + month + day + "1200.nc"
                        metadata['downloadLinkPolstere'] = "ftp://mftp.cmems.met.no/Core/SEAICE_GLO_SEAICE_L4_NRT_OBSERVATIONS_011_001/METNO-GLO-SEAICE_CONC-SOUTH-L4-NRT-OBS/" + year + "/" + month + "/" + "ice_conc_sh_polstere-100_multi_" + year + month + day + "1200.nc"
                        metadata['thumbnail'] = sic_south_thumbnail

                    hash = hashlib.md5(json.dumps(metadata)).hexdigest() #hash content data
                    if hash in current_guids:
                        current_guids_in_harvest.add(hash)
                    else:
                        ...
                        if collection == sst_thumbnail:
                            ...
                            obj = HarvestObject(job=harvest_job, guid=hash, extras=[
                                HOExtra(key='status', value='new'),
                                HOExtra(key='identifier', value=metadata['identifier']),
                                HOExtra(key='download_link_ease', value=metadata['downloadLinkEase']),
                                HOExtra(key='download_link_polstere', value=metadata['downloadLinkPolstere']),
                                HOExtra(key='dataset_name', value=metadata['datasetname']),
                                #HOExtra(key='original_document', value=xml_string),
                                HOExtra(key='original_metadata', value=json.dumps(metadata)),
                                HOExtra(key='original_format', value='netCDF')
    ...
    def _create_package_dict(self, harvest_object, context, previous_object):
        ...
        spatial_json="{\"type\":\"Polygon\",\"crs\":{\"type\":\"EPSG\",\"properties\":{\"code\":4326,\"coordinate_order\":\"Long,Lat\"}},\"coordinates\":["+str(metadata['Coordinates'])+"]}"

        try:
            thumbnail= metadata['thumbnail']
            extras_dict = self._generateExtrasDict(name=dataset_name.upper(),metadata=metadata,thumbnail=thumbnail, spatial=spatial_json)
        except Exception:
            extras_dict = self._generateExtrasDict(name=dataset_name.upper(),metadata=metadata,spatial=spatial_json)
        ...
        ###### FINISHED ADD EXTRAS
        # Build the package dict
        package_dict = self._get_package_dict(metadata, harvest_object, extras_dict, tags_dict)
        ...
        self._create_dataset(harvest_object, previous_object, context, package_dict, tag_schema, metadata)

    def _create_dataset(self, harvest_object, previous_object, context, package_dict, tag_schema, metadata):
            ...
            try:
                ...
                for key,value in metadata.iteritems():                    
                    ...
                    if key == 'downloadLink':
                        ...
                        resource_dict['description'] = 'Download the netCDF from CMEMS. NOTE: DOWNLOAD REQUIRES LOGIN'
                    ...
                    if key == 'downloadLinkPolstere':
                        ...
                        resource_dict['package_id'] = package_id
                        resource_dict['url'] = str(value)
                        resource_dict['name'] = 'Product Download (Polar Stereographic)'
                        resource_dict['description'] = 'Download the netCDF from CMEMS. NOTE: DOWNLOAD REQUIRES LOGIN'
                        resource_dict['format'] = 'netcdf'
                        resource_dict['mimetype'] = 'application/x-netcdf'
                        plugins.toolkit.get_action('resource_create')(context,resource_dict)
                ...
                self._save_object_error('Error: %s' % str(e), harvest_object, 'Import')
                return False
    ...
    def _get_package_dict(self, metadata, harvest_object,extras_dict, tags_dict):
        '''
        ...
        for tag in tags:
            if tag.get('name') == "SST":
                ...
                notes = "Daily analysis of sea surface temperature (SST), based on measurements from several satellite and in situ SST datasets, for the global ocean and some lakes."
            elif tag.get('name') == "observation":
                for tag2 in tags:
                    if tag2.get('name') == "antarctic":
                        dataset_name = "Antarctic Ocean Observed Sea Ice Concentration"
                        notes = "Daily sea ice concentration at 10km resolution in polar stereographic and EASE grid projections covering the Southern Hemisphere."
                    elif tag2.get('name') == "arctic":
                        dataset_name = "Arctic Ocean Observed Sea Ice Concentration"
                        notes = "Daily sea ice concentration at 10km resolution in polar stereographic and EASE grid projections covering the Northern Hemisphere."
