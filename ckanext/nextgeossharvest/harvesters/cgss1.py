...
from cgss1_collections import COLLECTION_DESCRIPTIONS

log = logging.getLogger(__name__)

URL_TEMPLATE = 'http://www.vito-eodata.be/openSearch/findProducts.atom?collection=urn:eop:VITO:CGS_S1_{}&start={}&end={}&count=500'  # noqa: E501
DATE_FORMAT = '%Y-%m-%d'
...
class CGSCollection(object):
    ...
    def get_tags(self):
        if len(self.product_type.split('_')) == 3:
            return ['VITO', 'sentinel-1', self.product_type.split('_')[0], self.product_type.split('_')[1]]  # noqa: E501
        else:
            return ['VITO', 'sentinel-1', self.product_type.split('_')[0]]  # noqa: E501

    def get_name(self):
        if len(self.product_type.split('_')) == 3:
            return 'CGS S1 {} {} {}'.format(self.product_type.split('_')[0], self.product_type.split('_')[1], self.product_type.split('_')[2])  # noqa: E501
        else:
            return 'CGS S1 {} {}'.format(self.product_type.split('_')[0], self.product_type.split('_')[1])  # noqa: E501

    def __str__(self):
        return 'CGS_S1_{}'.format(self.product_type)


class CGSHarvester(OpenSearchHarvester, NextGEOSSHarvester):
    """
    ...
    def info(self):
        return {
            'name': 'cgs-s1',
            'title': 'VITO CGS S1 Harvester',
            'description': 'A Harvester for the CGS S1 products processed on VITO premises'  # noqa: E501
        }
    ...
    def gather_stage(self, harvest_job):
        ...
        log.debug('VITO CGS S1 harvester gather_stage for job: %r', harvest_job)  # noqa E501
    ...
    def _parse_collection_from_identifier(self, identifier):
        ...
        if 'SLC_L1' in collection_name:
            return CGSCollection('SLC_L1')
        elif 'GRD_SIGMA0_L1' in collection_name:
            return CGSCollection('GRD_SIGMA0_L1')
        elif 'GRD_L1' in collection_name:
            return CGSCollection('GRD_L1')
