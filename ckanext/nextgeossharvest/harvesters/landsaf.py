...
from ckanext.nextgeossharvest.lib.landsaf_base import LandsafBaseHarvester
...
class LandsafHarvester(LandsafBaseHarvester, NextGEOSSHarvester,
                        HarvesterBase):
    """A Harvester for Landsaf Products."""
    implements(IHarvester)

    def info(self):
        return {
            'name': 'Landsaf',
            'title': 'Landsaf Harvester',
            'description': 'A Harvester for Landsaf Products'
        }
    ...
    def gather_stage(self, harvest_job):
        log = logging.getLogger(__name__ + '.Landsaf.gather')
        log.debug('LandsafHarvester gather_stage for job: %r', harvest_job)
        ...
        base_url = 'https://landsaf2.ipma.pt'

        url_template = ('{base_url}/geonetwork/srv/eng/xml.search?' +
                        '{time_query}' +
                        '&from=1&to={limit}' +
