...
class TestProvaVCollection(TestCase):
    ...
    def test_l2a_collection_str(self):
        collection = L2AProbaVCollection(
            ProductType.L2A, Resolution(333, Units.METERS))
    ...
    def test_get_description(self):
        collection = L2AProbaVCollection(
            ProductType.L2A, Resolution(333, Units.METERS))
        self.assertEqual(collection.get_description(
...
class TestSProvaVCollection(TestCase):
    ...
    def test_l2a_collection_str(self):
        collection = SProbaVCollection(
            1, ProductType.TOA, Resolution(333, Units.METERS), False)
    ...
    def test_get_tags(self):
        collection = SProbaVCollection(
            1, ProductType.TOA, Resolution(333, Units.METERS), False)
        self.assertListEqual(collection.get_tags(), [
                             'Proba-V', 'S1-TOA', '333M'])
...
class TestSProbavHarvester(TestCase):
    ...
    def test_parse_S_content(self):
        ...
        bbox = [[lng_min, lat_max],
                ...
                [lng_min, lat_min],
                [lng_min, lat_max]]
        expected_spatial = {"type": "Polygon", "crs": {"type": "EPSG", "properties": {
            "code": 4326, "coordinate_order": "Long,Lat"}}, "coordinates": [bbox]}
    ...
    def test_get_metalink_file_entries(self):
        ...
        self.assertEqual(
            metalink_entries[0]['name'], "PROBAV_S1_TOA_X00Y00_20180101_100M_V101.HDF5")
        self.assertTrue(all(file_entry['name'].endswith(
            '.HDF5') for file_entry in metalink_entries))
...
class TestProbavHarvester(TestCase):
    ...
    def test_generate_harvest_url(self):
        url = self.harvester._generate_harvest_url(
            'PROBAV_L2A_333M_V001', date(2018, 1, 1), date(2018, 1, 2))
        self.assertEqual(
            url, 'http://www.vito-eodata.be/openSearch/findProducts.atom?collection=urn:ogc:def:EOP:VITO:PROBAV_L2A_333M_V001&platform=PV01&start=2018-01-01&end=2018-01-02&count=500')
    ...
    def test_get_entries_from_results(self):
        ...
        entry_iter = self.harvester._parse_open_search_entries(
            entries_from_file)
        self.maxDiff = None
        self.assertEqual(entry_iter[0].identifier.string,
    ...
    def test_gather_L2A(self):
        ...
        self.assertEqual(
            firts_havest_object['guid'], 'urn:ogc:def:EOP:VITO:PROBAV_L2A_333M_V001:PROBAV_CENTER_L2A_20180101_005544_333M:V101')
        self.assertEqual(len(harvest_objects), 158)

    def test_gather_L3(self):
        harvest_objects_iterator = self.harvester._gather_L3(
            'http://www.vito-eodata.be/openSearch/findProducts.atom?collection=urn:ogc:def:EOP:VITO:PROBAV_S1-TOA_333M_V001&platform=PV01&start=2018-01-01&end=2018-01-02&count=500', auth=('nextgeoss', 'nextgeoss'))
        harvest_objects = list(harvest_objects_iterator)
        first_havest_object = harvest_objects[0]
        self.assertEqual(
            first_havest_object['guid'], 'urn:ogc:def:EOP:VITO:PROBAV_S1-TOA_333M_V001:PROBAV_S1-TOA_20180101_333M:V101:PROBAV_S1_TOA_X00Y00_20180101_333M_V101.HDF5')
    ...
    def test_create_harvest_object(self):
        harvest_object = self.harvester._create_harvest_object(
            'GUID:1', 'restart_date', 'content', extras={'k1': 1, 'k2': 2})
    ...
    def test_open_search_pages_from(self):
        ...
        self.assertEqual(open_search_first_page.find(
            'totalResults').string, '158')

    def test_get_xml_url(self):
        xml = self.harvester._get_xml_from_url(
            'https://www.vito-eodata.be/PDF/dataaccess?service=DSEO&request=GetProduct&version=1.0.0&collectionID=1000112&productID=267469925&ProductURI=urn:ogc:def:EOP:VITO:PROBAV_S1-TOA_333M_V001:PROBAV_S1-TOA_20180101_333M:V101&', auth=('nextgeoss', 'nextgeoss'))
    ...
    def test_generate_L3_guid(self):
        ...
        self.assertEqual(
            guid, 'urn:ogc:def:EOP:VITO:PROBAV_L2A_333M_V001:PROBAV_CENTER_L2A_20180101_005544_333M:V101:PROBAV_S1_TOA_20180101_333M_V101')
    ...
    def test_create_contents_json(self):
        ...
        self.assertDictEqual(json.loads(json_string), {
                             'opensearch_entry': 'os entry', 'file_entry': 'metalink entry'})
    ...
    def test_parse_L2A_content(self):
        ...
        expected_spatial = {"type": "Polygon", "crs": {"type": "EPSG", "properties": {"code": 4326, "coordinate_order": "Long,Lat"}}, "coordinates": [
            [[145.476, 65.071], [165.962992, 65.071], [165.962992, 40.341], [145.476, 40.341], [145.476, 65.071]]]}
        spatial = json.loads(parsed_content['spatial'])
        del parsed_content['spatial']
        self.maxDiff = None
        self.assertDictEqual(parsed_content, expected_parsed_content)
    ...
    def test_get_metadata_url(self):
        ...
        self.assertEqual(
            metadata_url, "https://www.vito-eodata.be/PDF/dataaccessMdXML?mdmode=hma&collectionID=1000126&productID=267446487&fileName=PV_CENTER_L2A-20180101005544_333M_V101.xml")
